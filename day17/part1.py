...
def _main():
    ...
    for _ in range(n_cycles):
        ...
        for x, y, z in itertools.product(range(x_min, x_max + 1), range(y_min, y_max + 1), range(z_min, z_max + 1)):
            # Get number of active neighbours
            neighbours_relative_coordinates = set(itertools.product(*[range(-1, 2)] * 3)) - {(0, 0, 0)}
            active_neighbours_cnt = 0
            for x_prime, y_prime, z_prime in neighbours_relative_coordinates:
                if (x + x_prime, y + y_prime, z + z_prime) in active_cubes:
                    active_neighbours_cnt += 1

            current_cube_coordinates = (x, y, z)
